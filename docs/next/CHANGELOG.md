...
### Added
- The sidebar now keeps a notification history under the agent panel, so a toast that fired while you were away is still there when you get back. Each entry names the agent, the local time, and what it said, and clicking one jumps to the pane it came from. Herdr records the state changes it notices and anything sent through `herdr notification show`, which now attributes itself to the calling pane's agent through `HERDR_PANE_ID` or `--pane`. The section takes only the rows it has entries for and none at all while the history is empty; `[ui.sidebar.notifications]` tunes its size and cap. `herdr notification list` and `herdr notification clear` read and empty the same history.
