...
`--pane` names the pane the notification speaks for, so the history can attribute it to that agent. It defaults to `HERDR_PANE_ID`, which means an agent calling this from inside its own pane is attributed automatically. A call from outside any pane is recorded as Herdr's own.

`notification list` reports shown notifications, newest first, with the agent, time, and source of each. `notification clear` empties the history. Both work regardless of `[ui.sidebar.notifications]`, which only controls whether the sidebar shows the same list.
