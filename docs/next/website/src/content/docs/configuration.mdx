...
- `state_elapsed` — how long the agent has been in its current state, rendered as `12초`, `3분`, or `1시간 20분`. Computed at render time from `state_changed_at_unix_ms`; omitted when the transition time is unknown, for example right after a cold restore.
