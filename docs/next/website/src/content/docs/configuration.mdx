...
Panes carry a title on their top border. A pane an agent has titled, or that you named
yourself, keeps that name; otherwise the border says what `[ui.pane_border]` asks for:
...
Tokens are `cwd`, `agent`, `state_icon`, `state_text`, `branch`, `pane`, `workspace`, `tab`,
`terminal_title`, `terminal_title_stripped`, and `$name` for values reported through
`herdr pane report-metadata`. Set `title = []` to leave borders blank.

A split pane already has a border to write on. An unsplit pane does not, so it is given a
single rule across its top — one line directly under the tab row — which is what
`show_when_single_pane` controls. Set it to `false` to leave a lone pane at full height;
splits keep their titles either way. `pane_borders = false` and `pane_outer_borders = false`
also remove that rule, since a lone pane's top edge is the outer frame, and so does
`title = []`, which would otherwise spend a line on a blank rule.

Every visible pane belongs to the same workspace and tab, so `workspace` and `tab` print the
same word on all of them; `cwd` is what actually differs.

The rule costs one terminal row. That matters most in mobile layout, where a single unsplit
pane is the norm and vertical space is scarcest — turn `show_when_single_pane` off there if
the row is better spent on output.

A detected agent is not a name, so it no longer takes the border to itself: it reaches the
title through the `agent` token, alongside whatever else you asked for. The older
`show_agent_labels_on_pane_borders` switch — and the Pane labels toggle in settings — now
turns that one token off rather than the whole title.
...
A toast is gone as soon as the next one lands. The notification section under the agent panel keeps what already fired, newest first, so you can see what called for you while you were away:
...
Each entry takes two rows: the local time and the agent that raised it, then what it said. Herdr records the state changes it notices — an agent asking for attention or finishing — and anything sent through `herdr notification show`. A notification suppressed because you were already looking at that pane is not recorded, because nothing was announced.

The section grows only to what it has to show. `height` is the most rows it may take from the bottom of the sidebar, and an empty history takes none at all, so turning this on costs a quiet session nothing. Values below 3 hide the section, as does `enabled = false`. Both keep recording: `herdr notification list` reports the history either way, and `herdr notification clear` empties it.

Click an entry to jump to the pane it came from. Entries whose pane has since closed stay in the list and simply do not jump.

`limit` is how many notifications are kept in memory. The history is not saved across server restarts.
