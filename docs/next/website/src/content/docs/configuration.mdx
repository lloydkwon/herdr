...
row_gap = 0
rows = [
  ["state_icon", "machine", "workspace", "tab", "state_elapsed"],
  ["agent"],
]
...
- `state_elapsed` — how long the agent has been in its current state, rendered as `12s`, `3m`, `1h20m`, or `2d3h`. Computed at render time from `state_changed_at_unix_ms`; omitted when the transition time is unknown, for example right after a cold restore.
