...
`--pane` は通知の発信元となるペインを指定し、履歴がその通知をエージェントに紐づけられるようにします。デフォルトは `HERDR_PANE_ID` なので、自分のペインの中から呼び出したエージェントは自動的に紐づきます。ペインの外からの呼び出しは Herdr 自身の通知として記録されます。

`notification list` は表示された通知を新しい順に、エージェント・時刻・種別とともに返します。`notification clear` は履歴を空にします。どちらも `[ui.sidebar.notifications]` の設定に関係なく動作します。この設定はサイドバーに同じ一覧を表示するかどうかだけを制御します。
