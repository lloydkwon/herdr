...
ペインは上端の境界線にタイトルを表示します。エージェントがタイトルを付けたペインや、
自分で名前を付けたペインはその名前のままです。それ以外は `[ui.pane_border]` の指定に従います。
...
トークンは `cwd`、`agent`、`state_icon`、`state_text`、`branch`、`pane`、`workspace`、`tab`、
`terminal_title`、`terminal_title_stripped`、そして `herdr pane report-metadata` で報告された値を
指す `$name` です。`title = []` にすると境界線は空のままになります。

分割ペインには書き込める境界線がすでにあります。分割していない単一ペインにはないので、
上端に横罫線が 1 本だけ引かれます。タブ行のすぐ下に出るこの 1 行が
`show_when_single_pane` の対象です。`false` にすると単一ペインは全高のままになり、
分割ペインのタイトルはどちらでも変わりません。単一ペインの上端は外枠そのものなので、
`pane_borders = false` と `pane_outer_borders = false` でもこの罫線は消えます。
`title = []` の場合も、中身のない罫線に 1 行を使わないよう消えます。

画面に見えるペインはすべて同じワークスペースとタブに属するため、`workspace` と `tab` は
どのペインにも同じ文字を出します。実際に異なるのは `cwd` です。

この罫線は端末の 1 行を消費します。特に影響が大きいのはモバイル表示で、単一ペインが
基本かつ縦の余裕が最も少ない場面です。その 1 行を出力に使いたい場合は
`show_when_single_pane` を off にしてください。

検出されたエージェントは名前ではないので、境界線を独占しなくなりました。指定した他の
トークンと並んで `agent` トークンとして出ます。従来の `show_agent_labels_on_pane_borders`
スイッチと設定画面の Pane labels トグルは、タイトル全体ではなくこのトークンだけを消します。
...
トーストは次の通知が届いた時点で消えます。エージェントパネルの下の通知セクションは、すでに発生した通知を新しい順に残すので、席を外している間に何が呼んでいたのかを後から確認できます:
...
各エントリは 2 行を使います。1 行目にローカル時刻と通知を出したエージェント、2 行目にその内容が入ります。Herdr が検知した状態変化 — エージェントが入力を求めた、または完了した — と、`herdr notification show` で送られたものを記録します。すでにそのペインを見ていたために抑制された通知は、何も告知されていないので記録しません。

セクションは表示する内容の分だけ伸びます。`height` はサイドバーの下端から取れる最大行数で、履歴が空のときは 1 行も取りません。そのため、静かなセッションではこの機能を有効にしても何も失いません。3 未満の値と `enabled = false` はセクションを隠します。どちらの場合も記録は続き、`herdr notification list` は履歴を返し、`herdr notification clear` は履歴を空にします。

エントリをクリックすると、その通知を出したペインに移動します。ペインが閉じられている場合、エントリは一覧に残りますが移動はしません。

`limit` はメモリに保持する通知の数です。履歴はサーバーの再起動をまたいで保存されません。
