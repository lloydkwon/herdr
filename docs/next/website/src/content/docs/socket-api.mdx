...
`WorkspaceInfo` (returned by `workspace.list`, `workspace.get`, and the workspace mutation methods) exposes optional `git_branch`, `git_ahead`, and `git_behind` fields read from the same periodic git refresh cache that drives the sidebar; no git command runs per request. `git_branch` is absent when the workspace cwd is not a git repository or HEAD is detached. `git_ahead` and `git_behind` are always present together and absent together: `0`/`0` means the branch is in sync with its upstream, while absence means there is no upstream or the value has not been computed yet, for example when the Space sidebar layout has no `git_status` token.
...
`PaneInfo` and `AgentInfo` expose an optional `state_changed_at_unix_ms` field: the wall-clock unix millisecond timestamp of the last semantic agent state transition, recorded only when the state actually changes. It is omitted when Herdr does not know the transition time (cold restore, older state). Compute elapsed time on the consumer side and clamp negative differences to zero.
