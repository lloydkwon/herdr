...
窗格会在上边框显示标题。已被智能体命名的窗格,或你自己命名的窗格,保留原有名称;
其余则按 `[ui.pane_border]` 的配置显示:
...
可用记号有 `cwd`、`agent`、`state_icon`、`state_text`、`branch`、`pane`、`workspace`、`tab`、
`terminal_title`、`terminal_title_stripped`,以及指向 `herdr pane report-metadata` 上报值的
`$name`。设为 `title = []` 可让边框保持空白。

分割窗格本来就有可写的边框。未分割的单个窗格没有,因此会在其顶部单独画一条横线 ——
就在标签行正下方的这一行,由 `show_when_single_pane` 控制。设为 `false` 可让单个窗格
保持满高,分割窗格的标题不受影响。单个窗格的上边缘本身就是外框,所以
`pane_borders = false` 和 `pane_outer_borders = false` 同样会移除这条横线;
`title = []` 也会,以免为一条空白横线浪费一行。

屏幕上可见的窗格都属于同一个工作区和标签页,所以 `workspace` 和 `tab` 在每个窗格上都是
同样的文字,真正有区别的是 `cwd`。

这条横线会占用终端的一行。在移动端布局中影响最明显 —— 那里单个未分割窗格是常态,
纵向空间也最紧张。若更希望把这一行留给输出,请关闭 `show_when_single_pane`。

检测到的 agent 并不是名字,因此它不再独占边框,而是与你配置的其他记号一起,通过 `agent`
记号出现。原有的 `show_agent_labels_on_pane_borders` 开关以及设置中的 Pane labels 选项,
现在只关闭这一个记号,而不是整个标题。
...
下一条通知到达时，上一条 toast 就消失了。智能体面板下方的通知区保留已经发出的通知，最新的在最上面，这样你就能看到离开时是什么在叫你：
...
每条记录占两行：本地时间和发出通知的智能体，然后是通知内容。Herdr 记录它检测到的状态变化 —— 智能体需要输入或已完成 —— 以及通过 `herdr notification show` 发送的通知。因为你正看着那个面板而被抑制的通知不会被记录，因为它根本没有发出。

通知区只会长到够用为止。`height` 是它能从侧边栏底部占用的最大行数，历史为空时一行也不占，所以在安静的会话里开启这个功能不会有任何代价。小于 3 的值和 `enabled = false` 都会隐藏该区域。两种情况下记录都会继续：`herdr notification list` 仍然返回历史，`herdr notification clear` 会清空它。

点击某条记录会跳转到发出它的面板。面板已经关闭的记录仍留在列表中，只是不再跳转。

`limit` 是内存中保留的通知数量。历史不会跨服务器重启保存。
