...
`--pane` 指明通知代表哪个面板，这样历史记录才能把它归到对应的智能体名下。默认值取自 `HERDR_PANE_ID`，因此智能体在自己的面板中调用时会自动归属。来自面板之外的调用记为 Herdr 自身的通知。

`notification list` 按时间倒序返回已显示的通知，包含智能体、时间和来源。`notification clear` 清空历史。两者都与 `[ui.sidebar.notifications]` 无关，该配置只控制侧边栏是否显示同一份列表。
