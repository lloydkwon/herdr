...
use crate::api::schema::{
    AgentPromptParams, AgentRenameParams, AgentResumeArgsSetParams, AgentSendKeysParams,
...
impl App {
    ...
    pub(super) fn handle_agent_resume_args_set(
        &mut self,
        id: String,
        params: AgentResumeArgsSetParams,
    ) -> String {
        let agent = match self.set_agent_resume_args_target(&params.target, params.args) {
            Ok(agent) => agent,
            Err(err) => return encode_error_body(id, self.agent_resume_args_error_body(err)),
        };
